#!/usr/bin/env bash
# jellyfin-restore-config.sh [backup-file-name]
#
# Replaces Jellyfin's application data with one of the archives the backups
# service wrote.
#
#   ./jellyfin-restore-config.sh               list and ask
#   ./jellyfin-restore-config.sh <file-name>   restore that one
#
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than jellyfin.
set -Eeuo pipefail
...
if ! docker exec "$BKP" sh -c "set -eu
  ...
  echo "error: the restore failed part-way; $DATA may be incomplete. Restore another archive before using Jellyfin." >&2
  exit 1
fi
