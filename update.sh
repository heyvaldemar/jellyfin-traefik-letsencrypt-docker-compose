...
COMPOSE_FILE="jellyfin-traefik-letsencrypt-docker-compose.yml"
...
if [ "$new_major" != "$cur_major" ] && [ "$ALLOW_MAJOR" != "true" ]; then
  ...
  echo "  https://github.com/heyvaldemar/jellyfin-traefik-letsencrypt-docker-compose/releases/tag/$latest" >&2
  exit 3
fi
